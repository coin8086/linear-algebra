...
向量 $\mathbf{u}$ 和 $\mathbf{v}$ 正交的充要条件是
...
$$\hat{\mathbf{y}} = \mathrm{proj}_L \ \mathbf{y} = \frac{\mathbf{y} \cdot \mathbf{u}}{\mathbf{u} \cdot \mathbf{u}} \mathbf{u}$$
...
$$\begin{align*}
  a_k &= \frac{1}{\pi} \int_0^{2 \pi} f(t) \cos kt \mathrm{d}t \\
  b_k &= \frac{1}{\pi} \int_0^{2 \pi} f(t) \sin kt \mathrm{d}t
\end{align*}$$
...
本章介绍了向量的内积，以及定义在内积之上的向量长度、距离和正交性。在此基础上，有一系列概念和定理，最终得出最小二乘问题的解法。这些概念、定理和法则之间的关系如下

![向量内积、正交性与最小二乘法](./assets/06-relations-a.svg)

此外，本章还介绍了内积空间的概念，并指出一般内积空间与标准内积空间 $\mathbb{R}^n$ 并无本质差别，并以傅里叶级数作为例子介绍了内积空间的应用。
