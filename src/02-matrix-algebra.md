...
> 矩阵乘法是由复合线性变换引入的。设有线性变换 $T_1$ 、 $T_2$ ，其标准矩阵分别为 $A$ 、 $B$ ，则复合线性变换 $T_1[T_2(\mathbf{x})]$ 即 $A(B\mathbf{x})$ 。
...
把单位矩阵进行一次初等行变换，就得到*初等矩阵*。初等矩阵都可逆。
...
本章主要介绍了矩阵的各种运算，着重于矩阵乘法和矩阵的逆，以及分块矩阵的乘法与逆。本章还简要介绍了 $\mathbb{R}^n$ 子空间以及矩阵的列空间和零空间。

矩阵运算相关的概念和法则之间的关系如下

![矩阵运算](./assets/02-relations-a.svg)

分块矩阵运算相关的概念和法则之间的关系如下

![分块矩阵运算](./assets/02-relations-b.svg)

$\mathbb{R}^n$ 子空间相关的概念和定理之间的关系如下

![子空间](./assets/02-relations-c.svg)

矩阵的列空间和零空间相关的概念和定理之间的关系如下

![矩阵的列空间和零空间](./assets/02-relations-d.svg)
