...
- [2 矩阵代数](#2-矩阵代数)
  ...
  - [2.2 矩阵的逆](#22-矩阵的逆)
    ...
    - [求逆算法](#求逆算法)
    - [逆矩阵的另一种观点](#逆矩阵的另一种观点)
  - [2.3 可逆矩阵的特征](#23-可逆矩阵的特征)
    - [可逆矩阵定理](#可逆矩阵定理)
    - [可逆线性变换](#可逆线性变换)
  - [2.4 分块矩阵](#24-分块矩阵)
    - [分块矩阵的加发与标量乘法](#分块矩阵的加发与标量乘法)
    - [分块矩阵的乘法](#分块矩阵的乘法)
      - [矩阵乘积的不同观点](#矩阵乘积的不同观点)
    - [分块矩阵的逆](#分块矩阵的逆)
  - [2.5 矩阵因式分解](#25-矩阵因式分解)
    - [LU分解](#lu分解)
  - [2.8 $\\mathbb{R}^n$ 子空间](#28-mathbbrn-子空间)
    - [子空间的定义](#子空间的定义)
      - [线性组合与子空间](#线性组合与子空间)
    - [矩阵的列空间与零空间](#矩阵的列空间与零空间)
    - [子空间的基](#子空间的基)
      - [矩阵的零空间的基](#矩阵的零空间的基)
      - [矩阵的列空间的基](#矩阵的列空间的基)
  - [2.9 维数与秩](#29-维数与秩)
...
$\displaystyle A^{-1} = \frac{1}{ad - bc} \begin{bmatrix*}[r]
d & -b \\
-c & a \\
\end{bmatrix*}$
...
数 $ad - bc$ 称为 $A$ 的*行列式*，记作 $\det A = ad - bc$
...
由矩阵乘法的定义及 $AA^{-1} = I$ 可知 $A^{-1}$ 的列正是上面方程组的解。
...
设 $T: \mathbb{R}^n \to \mathbb{R}^n$ 为线性变换， $A$ 为 $T$ 的标准矩阵，则  可逆当且仅当 $A$ 是可逆矩阵，这时由 $S(\mathbf{x}) = A^{-1} \mathbf{x}$ 定义的线性变换是 $T$ 的逆，且它是唯一的。
...
矩阵 $A$ 的秩是 $A$ 的列空间的维数，记作 $\mathrm{rank} \, A$ ，即 $\mathrm{rank} \, A = \dim (\mathrm{Col} \, A)$

*定理 14* （秩定理）

若矩阵 $A$ 有 $n$ 列，则 $\dim (\mathrm{Col} \, A) + \dim (\mathrm{Nul} \, A) = n$
