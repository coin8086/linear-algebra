...
本章把向量从 $\mathbb{R}^n$ 中的有序数组推广到一般对象（如函数等），在此基础上延申了向量空间、线性相关性和线性变换等原先定义在 $\mathbb{R}^n$ 上的概念。这其中最重要的概念是基与坐标映射：前者生成一个向量空间，后者把一个向量空间同 $\mathbb{R}^n$ 联系起来，建立起一个同构关系。这些概念和定理的关系如下

![推广的向量空间](./assets/04-relations-a.svg)

本章还（比第二章更深入地）介绍了矩阵的零空间、列空间和行空间，并通过秩和秩定理把这三者联系起来。这些概念和定理的关系如下
