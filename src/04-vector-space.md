...
1. $\mathbf{u}, \mathbf{v}$ 之和 $\mathbf{u} + \mathbf{v}$ 属于 $V$
...
此外，对 $V$ 中任意向量 $\mathbf{u}$ 和任意标量 $c$ ，由以上公理可证
...
> 子空间的一个例子：设定义在闭区间 $[a, b]$ 上的所有实值函数组成的向量空间为 $V$ ，则定义在 $[a, b]$ 上的所有连续的实值函数组成的向量空间（记作 $C[a, b]$ ）是 $V$ 的一个子空间。
...
矩阵 $A$ 的列空间记作 $\mathrm{Col} \ A$ ，它是由 $A$ 的列的所有线性组合的集合。若 $A = [\mathbf{a}_1 \dots \mathbf{a}_n]$ ，则

$$\mathrm{Col} \ A = \mathrm{Span} \{ \mathbf{a}_1, \dots, \mathbf{a}_n \}$$
...
坐标映射是*同构的*。一般地，从一个向量空间 $V$ 映上到另一个向量空间 $W$ 的一对一线性变换称为从 $V$ 到 $W$ 上的一个*同构*。

对一个同构映射来说，每一个在 $V$ 中的计算可以完全相同的出现在 $W$ 中，反之亦然。例如，对于向量的线性相关关系，在 $V$ 中线性相关（或无关）的向量集合，映射到同构的空间 $W$ 中，也是线性相关（或无关）的。
...
> 若 $A$ 与 $B$ 行等价，则 $A$ 的行是 $B$ 的行的线性组合，同时 $B$ 的行也是 $A$ 的行的线性组合。

> 行变换不保持行的线性相关关系，但保持列的线性相关关系。
...
> 因为 $A$ 可逆等价于 $A^T$ 可逆，所以可逆矩阵定理可以扩展到 $A^T$ 上，例如： $A$ 可逆等价于 $\mathrm{Nul} \, A^T = \{ \mathbf{0} \}$ 。
...
易知 $\underset{B \mapsto C}{P}$ 是可逆矩阵（因为 $\mathbf{b}_1, \dots, \mathbf{b}_n$ 线性无关，且坐标映射是同构的），因此
...
设 $\mathcal{B} = \{ \mathbf{b}_1, \dots, \mathbf{b}_n \}$ 是 $\mathbb{R}^n$ 的一个基， $\mathcal{E} = \{ \mathbf{e}_1, \dots, \mathbf{e}_n \}$ 是 $\mathbb{R}^n$ 的标准基，则

$$\underset{B \mapsto E}{P} = P_B = [\mathbf{b}_1 \dots \mathbf{b}_n]$$
...
上式还可以这样理解：先把 $[\mathbf{x}]_B$ 通过 $P_B$ 转换成 $\mathbb{R}^n$ 标准基的坐标，再把这个坐标通过 $P_C^{-1}$ 转换为 $\mathcal{C}-$ 坐标。

以上各种坐标转换方法异曲同工，其中增广矩阵的算法最快。
...
本章把向量从 $\mathbb{R}^n$ 中的有序数组推广到一般对象（如函数等），在此基础上延申了向量空间、线性相关性和线性变换等原先定义在 $\mathbb{R}^n$ 上的概念。这其中最重要的概念是基与坐标映射：前者生成一个向量空间，后者把一个向量空间同 $\mathbb{R}^n$ 联系起来，建立起一个同构关系。这些概念和定理之间的关系如下

![推广的向量空间](./assets/04-relations-a.svg)

本章还（比第二章更深入地）介绍了矩阵的零空间、列空间和行空间，并通过秩和秩定理把这三者联系起来。这些概念和定理之间的关系如下

![矩阵的零空间、列空间和行空间](./assets/04-relations-b.svg)
